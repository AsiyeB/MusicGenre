...
def vocab_to_word2vec(fname, vocab:dict):
    ...
    embedding_dim = 300
    word_embeddings = np.zeros(shape=(len(vocab),embedding_dim), dtype='float32')
    count_missing = 0
    f = open("demofile3.txt", "w")
    for word, idx in vocab.items():
        if word == "<PAD>":
            word_embeddings[idx] = np.zeros(shape=(embedding_dim,) )
            continue

        embedding_vector = embeddings_index.get(word)
        if embedding_vector is not None:
            word_embeddings[idx] = embedding_vector
        else:
            f.write(word +'\n')
            count_missing += 1
...
if __name__ == '__main__':
    preprocess("amazon_music/", w2vmodel="glove.42B.300d.txt")
